...
var pins = [ "USR0", "USR1", "USR2", "USR3" ];
var DIO_SIZE = 4;
var PinsState = [0,0,0,0];
var AnalogInPins = ["P9_39","P9_40","P9_37","P9_38","P9_33","P9_36","P9_35"];
var AI_SIZE = 7;
var PWMPins = ["P9_14","P9_16","P8_19","P8_13"];
var PWM_SIZE = 4;
var TMR_SIZE = 5;
var AO_SIZE = 0;
...
function RunBotSpeak (command,socket) {
    ...
    var Lines = BotCode.length;
    var reply = "";
    var scripting = 0, ptr = 0,i,j;
    
    for (i = 0;i < Lines; i++) {
        switch (trim(BotCode[i])) {
            case "SCRIPT": {
                scripting = 1;
                ptr = 0;
                reply += "S_start\n";
            ...
            case "RUN": socket.write("Done\r\n");
            case "RUN&WAIT":
            case "DEBUG":  {
                j = 0;
                while (j < SCRIPT.length) {
                    var reply1 = ExecuteCommand(SCRIPT[j]);
                    console.log('executed '+SCRIPT[j]+' and replied '+reply1);
                    var goto = String(reply1).split(' ');
                    j = (goto[0] == "goto") ? Number(goto[1]): j + 1;
                    if (command !== 'RUN') socket.write('executed '+SCRIPT[j]+' and replied '+reply1 + '\r\n');
                    reply += reply1 + '\n';
                }
                reply += "ran " + SCRIPT.length + " lines of script\nDone";
...
function ExecuteCommand(Code) {
    if (trim(Code.slice(0,2)) == '//') return Code;  // just a comment
    if (trim(Code) =='') return '';   // blank line
    var command = trim(Code.slice(0,Code.indexOf(' ')));   // everything before the space
    ...
    switch (command) {
        ...
        case "WAIT": delay(1000*Retrieve(dest)); return Retrieve(dest);
        ...
        case "SYSTEM": return "no system commands";
        case "":
        case "end": return "";
...
function Assign(dest,value)  {
    ...
    switch (param[0]) {
        ...
        case "PWM":  {
            ...
            b.pinMode(PWMPins[param1],"out");
            if (param[0] == "PWM") value = 1 - value / 100;  // PWM goes from 0 - 100 percent but is backward - 0 = no wait - high speed
        ...
        case "TMR":  {
            ...
            return TIMERS[param1] = value + new Date().getTime();
        }
            // Read Only Variables
...
function Retrieve(source)  {
    ...
    switch (param[0]) {
        ...
        case "TMR": reply = (param1 < TMR_SIZE)?(new Date().getTime() - TIMERS[param1]):-1; break;
        default: {
            switch(param.length) {
                case 1: {
                ...
                default:  {
                    ...
                    reply = (param1 < VARS[param[0]+'_SIZE'])?VARS[param[0]][param1]:-1;
                    break;
                }
