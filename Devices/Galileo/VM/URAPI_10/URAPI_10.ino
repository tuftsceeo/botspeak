...
#define PWM_SIZE 6 //# of hardware PWM pins available
...
void RunBotSpeak(int length) {
 ...
 //     for (i = 0; i < VARS[3]; i++) {Serial.print(SCRIPT[i]); Serial.print(','); }
    ...
    case 'r': 
      ...
      while (i < VARS[3]) { //I think VARS[3] is the index of the final byte code in the script
        Reply = " ";
        ExecuteCommand(i,1); //execute the byte code, but it looks like this is exu
...
int Assign(int destIndex,int value, int s) {
      ...
      switch (HiByte) { //HiByte determines the type
        ...
        case 224: LoByte = (LoByte < DIO_SIZE) ? LoByte:DIO_SIZE;   TogglePort(DIOPins[VARS[LoByte]],value);  break; //Toggles port indicated by LoByte variable using the delay in "value"
